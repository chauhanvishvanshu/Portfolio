...
dots.forEach(dot => {
  dot.addEventListener('click', e => {
    e.preventDefault();
    document.querySelector(dot.dataset.target)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });
});
...
const navObserver = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    ...
    if (link) entry.isIntersecting ? link.classList.add('active') : link.classList.remove('active');
  });
}, { threshold: 0.6 });
    ...
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const timer = setInterval(() => {
          ...
          if (percentEl) { percentEl.textContent = current + '%'; percentEl.style.opacity = 1; percentEl.style.transform = 'translateY(0)'; }
        }, 30);
        obs.unobserve(el);
      }
    });
